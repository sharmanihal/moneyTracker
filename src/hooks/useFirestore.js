...
await ref.doc(id).delete()
    ...
    const updateDocument=async(id,updatedDocument)=>{
        dispatch({type:'IS_PENDING'})
        try{
           const doc= await ref.doc(id).update({...updatedDocument})
            dispatchIfNotCancelled({type:"UPDATE_DOCUMENT",payload:doc})
        }catch(err){
            dispatchIfNotCancelled({type:'ERROR',payload:"could not update the transaction"})
        }
    }
    useEffect(()=>{
        return()=>{
